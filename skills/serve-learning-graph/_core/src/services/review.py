...
Responsibilities:
    - get_queue: build the Ebbinghaus review queue and group it for display
      ...
      priority sort. Mostly read-only, with one deliberate write: mastered nodes
      whose effective mastery decayed below threshold are demoted to
      needs_review so all display surfaces agree (docs/21 B7).
...
def _demote_if_decayed(row: dict, user_id: str) -> None:
    """
    Demote a mastered node whose effective (time-decayed) mastery has dropped
    below its threshold to needs_review (docs/21 B7), so every display surface
    (CLI / print / GUI) shows the same truth.
    """
    state = db.get_state(row.get("node_id"), user_id)
    if not state or state.get("status") != "mastered":
        return
    threshold = row.get("mastery_threshold") or 0.80
    em = dag_utils.effective_mastery(
    ...
    log.info("Node %s demoted to needs_review (effective %.2f < %.2f)",
             row["node_id"], em, threshold)
...
def get_queue(user_id: str = "default", include_future: bool = True) -> dict:
    ...
    for row in reviews:
        _demote_if_decayed(row, user_id)
...
def _update_anchor_effectiveness(node_id: str, user_id: str, score: float) -> None:
    """
    EMA write-back of how well the mnemonic anchors held up in this review
    (docs/23 4b): new = 0.7*old + 0.3*score, first observation = score.
    """
    for anchor in db.get_mnemonic_anchors(node_id=node_id, user_id=user_id):
        old = anchor.get("effectiveness")
        new = score if old is None else 0.7 * float(old) + 0.3 * score
        db.update_mnemonic_anchor(anchor["id"], effectiveness=round(new, 4))
...
def finish_review(
    ...
    if review_id:
        ...
        # Round and interval come from the same call so they can never disagree
        # (docs/21 B3) — mirrors agents.reviewer.run_review_loop.
        next_round, interval_days = dag_utils.next_review_state(
            review.get("review_round", 1), total_score, threshold
        )
        db.complete_review(
            review_id=review_id,
            score=total_score,
    ...
    if not passed and node_id:
        # A failed scheduled review demotes previously-held knowledge to
        # needs_review (docs/21 B7); _finalize_exam wrote "learning".
        state = db.get_state(node_id, user_id) or {}
        db.upsert_state(
            node_id=node_id,
            status="needs_review",
            raw_score=state.get("raw_score", total_score),
            stability=state.get("stability", 1.0),
            last_reviewed=state.get("last_reviewed"),
            next_review=state.get("next_review"),
            review_count=state.get("review_count", 0),
    ...
    if node_id:
        _update_anchor_effectiveness(node_id, user_id, total_score)
