...
Key Design Decisions:
    ...
    - Error notebook entries are written for questions scoring below the node's
      mastery threshold, so strict (critical) nodes log borderline misses too.
...
6. 如果提供了"历史错题"，30%-50% 的题目应针对这些薄弱点（换角度重新考查，
   不要原题照搬），并在该题上输出 "targets_error": true

输出规则：
- 以 JSON 格式输出，包含 questions 数组
- 每道题包含：question_type, question, options(仅选择题), expected_answer, source_section(大纲小节序号，扩展题为null), is_expansion, targets_error(可选)
...
def generate_questions(
    ...
    error_entries: Optional[list[dict]] = None,
    reviewer_feedback: Optional[list[str]] = None,
) -> list[dict]:
    """
    Generate exam questions based on the node and its outline sections.

    error_entries: historical error_notebook rows — when present, 30-50% of the
    questions target those weak spots and are marked origin='error_driven'
    (docs/23 4b). reviewer_feedback: issues from a rejected reverse validation,
    injected on regeneration (docs/23 4a).
    ...
    if error_entries:
        lines = []
        for e in error_entries[:8]:
            lines.append(
                f"- 题目：{e.get('question', '')}（错误类型：{e.get('error_type') or '—'}；"
                f"正确答案要点：{(e.get('correct_answer') or '')[:120]}）"
            )
        errors_text = "\n\n## 历史错题（薄弱点）\n" + "\n".join(lines)
    ...
    # Normalise fields, dropping malformed entries up front so a partial batch
    # can never be persisted after create_exam (docs/21 B5).
    questions: list[dict] = []
    for q in raw_questions:
        if (
            not isinstance(q, dict)
            or not str(q.get("question") or "").strip()
            or not str(q.get("expected_answer") or "").strip()
        ):
            log.warning("Dropping malformed generated question: %r", q)
            continue
        ...
        q["origin"] = "error_driven" if q.pop("targets_error", False) else "outline"
        questions.append(q)

    if not questions:
        raise ValueError("Question generation returned no valid questions.")
...
# ── Reverse Validation (docs/23 4a) ────────────────────────────────────────────

EXAM_REVERSE_SYSTEM = """\
你是一名严格的审题人。给你一个知识点、它的学习大纲和一套候选考题，请独立审查：

1. 覆盖度：题目是否覆盖了大纲的主要小节？是否有大段内容完全没被考查？
2. 答案正确性：每题的 expected_answer 是否准确、完整、无事实错误？
3. 难度匹配：题目难度是否与节点严格度（critical/standard/familiarity）相称？

输出 JSON：
{
  "approved": true/false,
  "issues": ["问题描述", ...],
  "corrections": {"题目序号(从1计)": "修正后的 expected_answer", ...}
}
仅当存在实质性问题时才拒绝（approved=false）；轻微措辞问题请直接通过。
corrections 只在你能给出确定的修正答案时填写。
""".strip()
...
def validate_questions(
    ...
    """
    Reverse-validate a generated question set (coverage / answer correctness /
    difficulty vs strictness). Degrades open: a broken validator approves, so
    the exam path never hard-fails on the quality gate.

    Returns {approved: bool, issues: [str], corrections: {index_str: answer}}.
    """
    outline_text = "\n".join(
        f"§{s['index']} {s['title']}: {s.get('content', '')[:200]}"
        for s in outline_sections
    )
    questions_text = "\n".join(
        f"{i}. [{q.get('question_type', 'short_answer')}] {q['question']}\n"
        f"   标准答案：{q['expected_answer']}"
        for i, q in enumerate(questions, 1)
    )
    user_prompt = (
        f"## 知识点\n**{node['title']}**（严格度：{node.get('strictness_level', 'standard')}）\n"
    ...
    try:
        result = llm.call_json(EXAM_REVERSE_SYSTEM, user_prompt, max_tokens=2048, model=model)
        if not isinstance(result, dict):
            raise ValueError("Unexpected validation response type")
    except Exception as e:
        log.warning("Exam reverse validation failed (%s) — approving by default", e)
...
def spot_check_scores(
    ...
    """
    Re-score the most borderline answered questions (closest to the node's
    mastery threshold). Conservative overwrite: only when the recheck diverges
    from the original score by more than 0.2 (docs/23 4a).

    Returns the list of adjustments [{question_id, old_score, new_score}].
    """
    threshold = node.get("mastery_threshold", config.MASTERY_THRESHOLDS.get(
        node.get("strictness_level", "standard"), 0.80
    ))
    rows = [r for r in db.get_exam_questions(exam_id) if r.get("score") is not None]
    rows.sort(key=lambda r: abs(r["score"] - threshold))

    adjustments: list[dict] = []
    for row in rows[:max(0, sample_n)]:
        recheck = score_answer(
            question=row["question"],
            expected_answer=row["expected_answer"],
            user_answer=row.get("user_answer") or "（未作答）",
            strictness=node.get("strictness_level", "standard"),
            model=model,
        )
        new_score = float(recheck.get("score", row["score"]))
        if abs(new_score - row["score"]) > 0.2:
            db.answer_exam_question(
                row["id"], user_answer=row.get("user_answer") or "（未作答）",
                score=new_score,
            )
            adjustments.append({
                "question_id": row["id"],
            ...
            log.info("Spot check overrode score for %s: %.2f → %.2f",
                     row["id"], row["score"], new_score)
    return adjustments
...
def _finalize_exam(
    ...
    for q in scored_questions:
        q_score = q.get("score")
        # A question is a weak spot when it falls below the node's own mastery
        # bar, not a global 0.6 (docs/21 B4).
        if q_score is None or q_score >= threshold:
            continue
