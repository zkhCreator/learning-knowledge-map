...
What this file does NOT do:
    - Teaching / Socratic dialogue (that lives in src/agents/teacher.py)
    - First-time exam (that lives in src/agents/examiner.py)
    - DB schema or migrations (that lives in src/data/database.py)
    - Ebbinghaus math (that lives in src/domain/dag.py)
...
def run_review_loop(
    ...
    if review:
        ...
        # (docs/21 B3): badly failed resets to round 1, partial keeps the round.
        next_round, interval_days = dag_utils.next_review_state(
            review_round, total_score, threshold
        )
        db.complete_review(
            review_id=review["id"],
            score=total_score,
    ...
    if not passed:
        # A failed *scheduled* review means previously-held knowledge slipped:
        # demote to needs_review (distinct from never-mastered "learning",
        # docs/21 B7). _finalize_exam wrote "learning"; overwrite it here.
    ...
    # EMA write-back of anchor effectiveness for this review (docs/23 4b);
    # mirrors services.review._update_anchor_effectiveness for the web path.
    for anchor in db.get_mnemonic_anchors(node_id=node_id, user_id=user_id):
        old_eff = anchor.get("effectiveness")
        new_eff = total_score if old_eff is None else 0.7 * float(old_eff) + 0.3 * total_score
        db.update_mnemonic_anchor(anchor["id"], effectiveness=round(new_eff, 4))
