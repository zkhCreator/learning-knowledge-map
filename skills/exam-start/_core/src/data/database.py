...
# Schema version stamped into PRAGMA user_version (docs/21). Bump together with
# a new migration step in _migrate().
SCHEMA_VERSION = 2

# DB paths already migrated by this process — one lazy check per path.
...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    v0 → v1: baseline — replay SCHEMA_SQL (CREATE TABLE IF NOT EXISTS fills in
    any missing tables).
    v1 → v2: exam_validations table (via the replay) + exam_questions.origin
    column (docs/23).
    """
    ...
    if current < 2:
        _add_column_if_missing(conn, "exam_questions", "origin", "TEXT")
...
CREATE TABLE IF NOT EXISTS exam_questions (
    ...
    origin          TEXT,          -- 'outline' | 'error_driven' (NULL = legacy/outline)
    created_at      TEXT NOT NULL
);

-- Reverse-validation audit trail for generated exams (docs/23)
CREATE TABLE IF NOT EXISTS exam_validations (
    id          TEXT PRIMARY KEY,
    exam_id     TEXT NOT NULL REFERENCES exam_attempts(id),
    verdict     TEXT NOT NULL,
    -- 'approved' | 'approved_after_regen' | 'rejected_with_corrections'
    issues      TEXT,              -- JSON list of reviewer issues
    created_at  TEXT NOT NULL
);
        ...
        conn.execute(
            """INSERT INTO exam_questions
               (id, exam_id, question_type, question, options, expected_answer,
                user_answer, score, source_section, is_expansion, origin, created_at)
               VALUES (
                :id,:exam_id,:question_type,:question,:options,:expected_answer,
                :user_answer,:score,:source_section,:is_expansion,:origin,:created_at
            )""",
...
def add_exam_validation(exam_id: str, verdict: str, issues: list | None = None) -> dict:
    row = {
        "id": _id(),
        "exam_id": exam_id,
        "verdict": verdict,
    ...
    with get_connection() as conn:
        conn.execute(
            "INSERT INTO exam_validations VALUES (:id,:exam_id,:verdict,:issues,:created_at)",
            row,
        )
    return row


def list_exam_validations(exam_id: str) -> list[dict]:
    with get_connection() as conn:
        rows = conn.execute(
            "SELECT * FROM exam_validations WHERE exam_id=? ORDER BY created_at",
            (exam_id,),
        ).fetchall()
    return [dict(r) for r in rows]
