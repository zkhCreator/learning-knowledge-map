...
def generate_outline(
    ...
    # ── Persist mnemonic anchors ───────────────────────────────────────────────
    # Always clear old anchors first: a regeneration without a mnemonic strategy
    # must not leave anchors from the previous section layout behind (docs/21 B6).
...
def chat_turn(
    ...
    # Inject stored anchors from the DB as well, so old sessions and outlines
    # regenerated without inline mnemonic fields still surface the learner's
    # memory cues (docs/23 4b).
    anchors = db.get_mnemonic_anchors(
        node_id=node["id"], user_id=session.get("user_id", "default")
    )
    if anchors:
        anchor_lines = []
        for a in anchors:
            loc = f"（{a['palace_location']}）" if a.get("palace_location") else ""
            anchor_lines.append(
                f"§{a.get('section_index', '?')} [{a.get('strategy', '')}]{loc} {a.get('content', '')}"
            )
        outline_text += "\n\n## 学习者的助记锚点（引导回忆时引用）\n" + "\n".join(anchor_lines)
