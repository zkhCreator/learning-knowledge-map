...
from src.agents.examiner import (
    generate_questions,
    score_answer,
    validate_questions,
    _finalize_exam,
...
def _mnemonic_view(node_id: str, user_id: str) -> Optional[dict]:
    """Client-safe pre-exam recall cue (docs/23 4b). Never includes answers."""
...
def start_exam(node: str, user_id: str = "default", model: Optional[str] = None) -> dict:
    ...
    # Past errors steer 30-50% of the questions toward known weak spots; a
    # first exam has none and behaves exactly as before (docs/23 4b).
    error_entries = db.list_errors(user_id=user_id, node_id=node_id)

    questions_data = generate_questions(
        node_row, sections, model=model, error_entries=error_entries
    )

    # Forward/Reverse quality gate (docs/23 4a): one regeneration with the
    # reviewer's issues, then inline corrections as the last resort.
    verdict = None
    validation = None
    if config.EXAM_VALIDATE:
        validation = validate_questions(node_row, sections, questions_data, model=model)
        if validation["approved"]:
            verdict = "approved"
        else:
            questions_data = generate_questions(
                node_row, sections, model=model, error_entries=error_entries,
                reviewer_feedback=validation["issues"],
            )
            validation = validate_questions(node_row, sections, questions_data, model=model)
            if validation["approved"]:
                verdict = "approved_after_regen"
            else:
                for idx_str, corrected in validation["corrections"].items():
                    try:
                        questions_data[int(idx_str) - 1]["expected_answer"] = corrected
                    except (ValueError, IndexError):
                        pass
                verdict = "rejected_with_corrections"

    exam = db.create_exam(node_id=node_id, outline_id=outline_id, user_id=user_id)
    if verdict:
        db.add_exam_validation(exam["id"], verdict=verdict,
    ...
    return {
        ...
        "questions": questions,
        "total": len(questions),
        # Pre-exam recall cue (pure DB read; None without profile/anchors).
        "mnemonic": _mnemonic_view(node_id, user_id),
    }
...
def get_exam_view(exam_id: str, user_id: str = "default") -> dict:
    ...
    return {
        ...
        # Pre-exam recall cue for the GUI (pure DB read, no answer leakage).
        "mnemonic": _mnemonic_view(exam["node_id"], user_id),
    }
