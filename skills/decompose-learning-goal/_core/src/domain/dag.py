...
def next_review_state(review_round: int, score: float, threshold: float) -> tuple[int, int]:
    """
    Single source of truth for review-round accounting (docs/21 B3).

    Given the round of the review just completed, return
    (next_round, interval_days) so the persisted round and interval can never
    disagree:
        score < 0.5            → reset: round 1, round-1 interval
        0.5 ≤ score < threshold → keep the round, halve its base interval
        score ≥ threshold       → advance the round, next round's base interval
    """
    intervals = config.REVIEW_INTERVALS

    if score < 0.5:
        return 1, intervals[0]
    if score < threshold:
        idx = min(review_round - 1, len(intervals) - 1)
        return review_round, max(1, intervals[idx] // 2)
    next_round = review_round + 1
    idx = min(next_round - 1, len(intervals) - 1)
    return next_round, intervals[idx]
